...
%{account: account} =
  Repo.transaction(fn ->
    user =
      Repo.get_by(User, email: "seed@example.com") ||
        ...
        |> User.changeset(%{
          ...
          password_hash: Argon2.hash_pwd_salt("changeme123!"),
    ...
    Repo.get_by(Session, token_hash: :crypto.hash(:sha256, "seed-session-token")) ||
      %Session{}
      |> Session.changeset(%{
        user_id: user.id,
        token_hash: :crypto.hash(:sha256, "seed-session-token"),
        context: "seed",
        expires_at: DateTime.add(DateTime.utc_now(), 7 * 24 * 60 * 60, :second),
        encrypted_metadata: %{"note" => "Seed session"}
      })
      |> Repo.insert!()
    ...
    %{user: user, institution: institution, account: account}
  end)
